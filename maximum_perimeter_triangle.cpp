...
vector<int> maximumPerimeterTriangle(vector<int> sticks) {
    ...
    for (size_t i = 0;  i < sticks.size(); i++) {
        size_t indexInit = i + 2;

        if (indexInit >= sticks.size()) {
            break;
        }
        int sum = sticks[indexInit] + sticks[indexInit - 1];
        if (sum > sticks[indexInit -2]) {
            found = true;
            res.push_back(sticks[indexInit]);
            res.push_back(sticks[indexInit -1]);
            res.push_back(sticks[indexInit -2]);
            break;
        }
    }
